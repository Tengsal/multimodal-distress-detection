...
async def create_session(
    ...
    return await _process_session(session_obj, face_path, voice_path)


# ------------------------------------------------------------
# JSON-only endpoint (no media files)
# ------------------------------------------------------------

from fastapi import Request

@app.post("/sessions/json", status_code=status.HTTP_201_CREATED)
async def create_session_json(request: Request) -> Dict[str, Any]:
    """Accept a plain JSON session payload when no media files are attached."""
    ...
    print("DEBUG (JSON): Raw Session Data:", json.dumps(session_data, indent=2))
    session_obj = SessionIn(**session_data)
    print("Session received (JSON):", session_obj.session_id)

    return await _process_session(session_obj, face_path=None, voice_path=None)
...
async def _process_session(
    ...
    try:
        ...
        if face_path is not None and voice_path is not None:
